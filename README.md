...
## Kurulum ve Workspace (Çalışma Alanı) Hazırlığı

Bu depo (repository), gereksiz derleme dosyalarından arındırılmış (`.gitignore` ile temizlenmiş) saf kaynak kodları içerir. Bu nedenle GitHub'dan çektikten sonra standart bir ROS 2 çalışma alanı (workspace) olarak derlenmesi gerekir.

### 1. Repoyu Klonlama

Terminalinizi açın ve repoyu bilgisayarınıza (veya uçağın üzerindeki görev bilgisayarına) çekin:

```bash
git clone https://github.com/Eposby/otopilot_ws.git
cd otopilot_ws
...
Projenin çalışması için **ROS 2 Humble** ve **MAVROS**'un sisteminizde kurulu olması gerekir. Ayrıca aşağıdaki Python ve sistem kütüphanelerini yükleyin:
...
### 3. Çalışma Alanını Derleme (Build)

İndirilen kodların ROS 2 tarafından tanınması ve mesaj paketlerinin oluşturulması için colcon ile derlenmesi şarttır:

```bash
# Önce mesaj paketini, sonra ana paketi derleyin
colcon build --packages-select iha_messages
colcon build --packages-select iha_otopilot --symlink-install

# Yeni derlenen paketleri sisteme tanıtın
...
*(Not: Düğümleri çalıştırmadan önce terminalde `source install/setup.bash` işlemini yaptığınızdan emin olun.)*

### Node Örnekleri (Kullanım Senaryoları)
...
**(Not: Tüm düğümlerin listesini ve kısa komutlarını `setup.py` dosyasındaki `entry_points` kısmında bulabilirsiniz.)**
